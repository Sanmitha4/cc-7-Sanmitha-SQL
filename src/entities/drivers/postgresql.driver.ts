...
export class PostgreSqlDriver implements IDatabaseDriver {
    ...
    private buildWhereClause(conditions: Record<string, unknown> | undefined, startIndex: number): string {
        const keys = conditions ? Object.keys(conditions) : [];
        if (keys.length === 0) return "";
        
        let idx = startIndex;
        const clause = keys.map((k) => `${k} = ${this.placeholder(idx++)}`).join(" AND ");
        return ` WHERE ${clause}`;
    }
    ...
    getUpdateQuery(tableName: string, columns: string[], conditions: Record<string, unknown>): string {
        const setClause = columns.map((col, i) => `${col} = ${this.placeholder(i + 1)}`).join(", ");
